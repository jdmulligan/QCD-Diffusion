"""
Define a simple UNet-based architecture, based on https://amaarora.github.io/posts/2020-09-13-unet.html
"""
...
class UNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.head(out)
        if self.retain_dim:
            out = torch.nn.functional.interpolate(out, size=self.out_sz, mode='bilinear', align_corners=False)
        return out
...
class Encoder(torch.nn.Module):
    def __init__(self, chs=(1, 64, 128, 256, 512)):
        super().__init__()
        self.enc_blocks = torch.nn.ModuleList([Block(chs[i], chs[i + 1]) for i in range(len(chs) - 1)])
